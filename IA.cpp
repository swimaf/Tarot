...
shared_ptr<ACarte> IA::appelerRoi(vector<shared_ptr<ACarte>> allRois) {

    list<shared_ptr<ACarte>> listRoi;
    copy(allRois.begin(), allRois.end(), back_inserter(listRoi));

    vector<shared_ptr<ACarte>> rois = ACarte::getRois(jeux);
    cerr << "Possède :" << endl;
...
vector<shared_ptr<ACarte>> IA::selectionCartesChien(int taille) {
    vector<shared_ptr<ACarte>> chien;

    vector<vector<shared_ptr<ACarte>>> formes;
...
shared_ptr<ACarte> IA::selectionCarteAJouer() {
    return NULL;
}
